...
class ControllerExtensionPaymentAvantgarde extends Controller {
    ...
    public function index() {
        $this->language->load('extension/payment/avantgarde');
        $data['button_confirm'] = $this->language->get('button_confirm');
        $this->load->model('checkout/order');
        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        if ($order_info) {

            $settings['payment_avantgarde_merchant_id'] = $this->config->get('payment_avantgarde_merchant_id');
            $settings['payment_avantgarde_merchant_key'] = $this->config->get('payment_avantgarde_merchant_key');
            $settings['payment_avantgarde_aggregator_id'] = $this->config->get('payment_avantgarde_aggregator_id');
            $settings['payment_avantgarde_domain_url'] = $this->config->get('payment_avantgarde_domain_url');
            $settings['payment_avantgarde_transaction_method'] = $this->config->get('payment_avantgarde_transaction_method');

            if (isset($settings["payment_avantgarde_merchant_id"]) && isset($settings["payment_avantgarde_merchant_key"]) &&
                    isset($settings["payment_avantgarde_aggregator_id"]) && isset($settings["payment_avantgarde_domain_url"]) &&
                    isset($settings["payment_avantgarde_transaction_method"])) {
                $merchant_id = trim($settings["payment_avantgarde_merchant_id"]);

                $merchant_key = trim($settings["payment_avantgarde_merchant_key"]);
                $aggregator_id = trim($settings["payment_avantgarde_aggregator_id"]);
                $transaction_method = (trim($settings["payment_avantgarde_transaction_method"]) == 'auth') ? 'AUTH' : 'SALE';

                $mobiledetect = new Mobiledetect();
                $channel = ($mobiledetect->isMobile()) ? 'MOBILE' : 'WEB';
                $payment_avantgarde_domain_url = trim($settings["payment_avantgarde_domain_url"]);

                $orderid = $this->session->data['order_id'];
                $cancelUrl = $this->url->link('extension/payment/avantgarde/callback');
                $successUrl = $this->url->link('extension/payment/avantgarde/callback');
                ...
                $data["action"] = $payment_avantgarde_domain_url;

                $data['me_id'] = $merchant_id;

                $txn_details = $aggregator_id . '|' . $merchant_id . '|' . $orderid . '|' . $amount . '|' . $order_info['payment_iso_code_3'] . '|' . $order_info['currency_code'] . '|' . $transaction_method . '|' . $successUrl . '|' . $cancelUrl . '|' . $channel;

                $data['txn_details'] = $this->encrypt($txn_details, $merchant_key, 256);

                $pg_details = '|||';
                $data['pg_details'] = $this->encrypt($pg_details, $merchant_key, 256);

                $card_details = '||||';
                $data['card_details'] = $this->encrypt($card_details, $merchant_key, 256);

                $cust_details = $order_info['firstname'] . '|' . $order_info['email'] . '|' . $order_info['telephone'] . '|' . $customer_id . '|' . $islogin;
                $data['cust_details'] = $this->encrypt($cust_details, $merchant_key, 256);

                $bill_details = $order_info['payment_address_1'] . '|' . $order_info['payment_city'] . '|' . $order_info['payment_zone'] . '|' . $order_info['payment_country'] . '|' . $order_info['payment_postcode'];

                $data['bill_details'] = $this->encrypt($bill_details, $merchant_key, 256);

                $ship_details = $order_info['shipping_address_1'] . '|' . $order_info['shipping_city'] . '|' . $order_info['shipping_zone'] . '|' . $order_info['shipping_country'] . '|' . $order_info['shipping_postcode'] . '||';

                $data['ship_details'] = $this->encrypt($ship_details, $merchant_key, 256);
                $item_details = '||';
                $data['item_details'] = $this->encrypt($item_details, $merchant_key, 256);
                $other_details = '||||';
                $data['other_details'] = $this->encrypt($other_details, $merchant_key, 256);

                $data["error"] = "";
            } else {
                $data["error"] = "Invalid Configuration";
            }
            return $this->load->view('extension/payment/avantgarde', $data);
        }
    }
    ...
    public function callback() {
        ...
        if (isset($_POST) && isset($_POST['txn_response']) && !empty($_POST['txn_response'])) {
            $post = $_POST;
            $settings['payment_avantgarde_merchant_id'] = $this->config->get('payment_avantgarde_merchant_id');
            $merchant_key = $settings['payment_avantgarde_merchant_key'] = $this->config->get('payment_avantgarde_merchant_key');
            $settings['payment_avantgarde_aggregator_id'] = $this->config->get('payment_avantgarde_aggregator_id');
            $settings['payment_avantgarde_domain_url'] = $this->config->get('payment_avantgarde_domain_url');
            $settings['payment_avantgarde_transaction_method'] = $this->config->get('payment_avantgarde_transaction_method');
        ...
        if (isset($return_elements['txn_response']['order_no'])) {
            $order_id = $return_elements['txn_response']['order_no'];
        } else {
            die('Illegal Access');
        }
        ...
        if ($order_info) {
            ...
            if (!empty($return_elements['txn_response']['res_code']) && ($return_elements['txn_response']['res_code'] != 'Success')) {
                ...
                $this->model_checkout_order->addOrderHistory($order_id, 10, $return_elements['txn_response']['res_message'], true);
            ...
            if ($status == 'Successful' || $status == 'successful') {
                ...
                if (!$order_info['order_status_id'] || $order_info['order_status_id'] != $order_status_id) {
                    $this->model_checkout_order->addOrderHistory($order_id, $order_status_id, $return_elements['txn_response']['res_message']);
                } else {
                    $this->model_checkout_order->addOrderHistory($order_id, $order_status_id, $return_elements['txn_response']['res_message']);
                }
